# -*- coding: utf-8 -*-
"""
Created on Fri Apr 28 09:57:09 2017
...
#    pjg = len(bin_plaintext)
    bin_plaintext = bin_plaintext.zfill(64)
#    tambahNul = 64 - pjg
    
    L0, R0 = IP(ia, bin_plaintext)
    R.append(R0)
    C0, D0 = PC1(bin_key)
    LEFT_ROT(C0, D0, K)
...
def ENCRYPT(ia, K, R, bin_plaintext, bin_key, bin_iv):
    itt = 0
    itt_plus = 1

    #print len(bin_plaintext)
    
    temp = bin_plaintext
    for w in range (0, len(bin_plaintext)):
#        helper[w] = bin_plaintext[w]
        bin_plaintext[w] = XOR(bin_plaintext[w], bin_iv).zfill(64)
        DES(itt, itt_plus, ia, K, R, bin_plaintext[w], bin_key)
        chiper_encrypt.append(bin_plaintext)

#    bin_plaintext = XOR(bin_plaintext, bin_iv).zfill(64)
...
def DECRYPT(ia, K, R, bin_plaintext, bin_key, bin_iv):
    itt = 0
    itt_plus = 1
    
    dummy = bin_plaintext
 
    DES(itt, itt_plus, ia, K, R, dummy, bin_key)        
    
    bin_plaintext = XOR(dummy, bin_iv).zfill(64)
    text_decrypt.append(bin_plaintext)
...
def start_des(messages, key, mode, ivector):
   ...
   # print "mode sekarang: ",mode
    ...
    for ia in range(len(bin_plaintext)):
        ...
        if mode == 'encrypt' or mode == 'ENCRYPT' :
            result = ENCRYPT(ia, K, R, bin_plaintext, bin_key, bin_iv)
        elif mode == 'decrypt' or mode == 'DECRYPT' :
            result = DECRYPT(ia, K, R, bin_plaintext, bin_key, bin_iv)
...
def terima_pesan():
    while True:
        ...
        if(data):
            ...
            data = start_des(data, kunci, "decrypt", ivector)
            #print data
            print data   
        else:
#            print "no more data"
...
try:
    ...
    while True:
        message=raw_input()
        kirim=nama+": "+message
        encrypted = start_des(kirim, kunci, "encrypt", ivector)
